...
const ShowcaseCard = React.memo<{
  ...
  <div
    ...
    className={`group rounded-[28px] overflow-hidden bg-white dark:bg-dark-paper cursor-pointer transition-all duration-200 ${
      isActive
        ? 'shadow-2xl ring-2 ring-ot-primary/30 dark:ring-dark-primary/30'
...
const ListCard = React.memo<{
  ...
  <div
    ...
    className={`group flex gap-4 py-5 cursor-pointer transition-colors ${
      isActive
        ? 'bg-ot-athens-gray dark:bg-dark-surface'
        : 'hover:bg-ot-athens-gray dark:hover:bg-dark-surface'
    }`}
  >
    ...
    <div className="flex flex-col justify-between flex-1 min-w-0">
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm font-semibold text-ot-charade dark:text-dark-text">
            {ratingLabel(restaurant.rating)}
          </span>
        ...
        {restaurant.address && (
          <p className="text-xs text-ot-manatee dark:text-dark-text-secondary truncate">
            {restaurant.address}
          </p>
        )}
      </div>
...
const EmptyState: React.FC<{ query: string; onReset: () => void }> = ({ query, onReset }) => (
  <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
    <div className="w-20 h-20 rounded-full bg-ot-athens-gray dark:bg-dark-surface flex items-center justify-center mb-5 shadow-inner">
      <svg
        className="w-9 h-9 text-ot-manatee dark:text-dark-text-secondary"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={1.5}
    ...
    <h3 className="text-lg font-semibold text-ot-charade dark:text-dark-text mb-2">
...
const SearchPage: React.FC = () => {
  ...
  const hasActiveFilters =
    selectedCity !== '' || selectedPriceRanges.length > 0 || ratingMin !== null || openNow;
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-ot-athens-gray dark:bg-dark-bg">
      <div className="flex-shrink-0 bg-ot-charade dark:bg-dark-paper px-4 pt-4 pb-4 shadow-lg">
        <div className="max-w-7xl mx-auto space-y-3">
          <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
            <a
              href="/"
              className="text-white font-extrabold text-lg tracking-tight mr-2 select-none"
            >
              Reservelt
            </a>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="relative">
                ...
                >
                  {TIME_OPTIONS.map((t) => (
                    <option key={t} value={t} className="text-ot-charade">
                      {t}
                    </option>
                  ))}
                </select>
              ...
              <div className="relative">
                ...
                >
                  {PARTY_SIZES.map((p) => (
                    <option key={p} value={p} className="text-ot-charade">
                      {p}
                    </option>
                  ))}
                </select>
            ...
            <div className="flex-1 flex items-center gap-2 bg-white dark:bg-dark-surface rounded-2xl px-4 py-2.5 shadow-md">
              ...
              {searchQuery && (
                <button
                  ...
                  className="text-ot-manatee dark:text-dark-text-secondary hover:text-ot-pale-sky flex-shrink-0"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
      ...
      <div className="flex-shrink-0 bg-white dark:bg-dark-paper border-b border-ot-iron dark:border-dark-border">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-2 py-2.5 overflow-x-auto scrollbar-hide">
          <div className="relative flex-shrink-0">
            ...
            <svg
              className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-ot-manatee dark:text-dark-text-secondary pointer-events-none"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2.5}
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              <div>
                ...
                <p className="text-xs text-ot-manatee dark:text-dark-text-secondary">
                  ...
                  {selectedPriceRanges.length > 0 &&
                    ` · ${selectedPriceRanges.map((p) => PRICE_SYMBOLS[p]).join(' ')}`}
                ...
                <div className="flex bg-ot-athens-gray dark:bg-dark-surface rounded-xl p-0.5 gap-0.5">
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-colors ${
                      viewMode === 'grid'
                        ? 'bg-white dark:bg-dark-paper shadow-sm text-ot-charade dark:text-dark-text'
                        : 'text-ot-manatee dark:text-dark-text-secondary hover:text-ot-pale-sky'
                    }`}
                  >
                    <svg
                      className="w-4 h-4"
        ...
        <div
          className={`${showMap ? 'flex-1' : 'hidden'} md:block md:w-[45%] md:flex-none flex-shrink-0`}
        >
          <MapView
